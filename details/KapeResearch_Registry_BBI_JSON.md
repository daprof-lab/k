# ⚙️ **Kape Research Registry BBI JSON**
### `File Name: KapeResearch_Registry_BBI_JSON.mkape`
...
* **Bulk Automated Parsing**: Run Kape Research Registry BBI JSON to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Kape Research Registry BBI JSON logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Kape Research Registry BBI JSON timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
```yaml
Description: 'RECmd: Convert BBI Registry hive to JSON for research'
