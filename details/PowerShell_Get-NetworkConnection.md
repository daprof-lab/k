# ⚙️ **Power Shell Get Network Connection**
### `File Name: PowerShell_Get-NetworkConnection.mkape`
...
* **Bulk Automated Parsing**: Run Power Shell Get Network Connection to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Power Shell Get Network Connection logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Power Shell Get Network Connection timeline outputs to isolate exactly when unauthorized scripts were loaded.
