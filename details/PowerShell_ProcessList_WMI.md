# ⚙️ **Power Shell Process List WMI**
### `File Name: PowerShell_ProcessList_WMI.mkape`
...
* **Bulk Automated Parsing**: Run Power Shell Process List WMI to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Power Shell Process List WMI logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Power Shell Process List WMI timeline outputs to isolate exactly when unauthorized scripts were loaded.
