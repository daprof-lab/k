# ⚙️ **Kape Research Registry SECURITY JSON**
### `File Name: KapeResearch_Registry_SECURITY_JSON.mkape`
...
* **Bulk Automated Parsing**: Run Kape Research Registry SECURITY JSON to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Kape Research Registry SECURITY JSON logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Kape Research Registry SECURITY JSON timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceFile% --kn ROOT --nl false --json %destinationDirectory% --jsonf SECURITY_ROOT.json
        ExportFormat: json
