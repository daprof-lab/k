# ⚙️ **Volatility Timeliner**
### `File Name: Volatility_timeliner.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Timeliner to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Timeliner logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Timeliner timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp timeliner --output=greptext --output-file %destinationDirectory%\volatility_timeliner.txt
        ExportFormat: greptext
