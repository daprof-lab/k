# ⚙️ **Volatility Modscan**
### `File Name: Volatility_modscan.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Modscan to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Modscan logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Modscan timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp modscan --output=greptext --output-file %destinationDirectory%\volatility_modscan.txt
        ExportFormat: greptext
