...
* **Bulk Automated Parsing**: Run Evtx Ecmd RDP to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Evtx Ecmd RDP logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Evtx Ecmd RDP timeline outputs to isolate exactly when unauthorized scripts were loaded.
