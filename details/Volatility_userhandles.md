# ⚙️ **Volatility Userhandles**
### `File Name: Volatility_userhandles.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Userhandles to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Userhandles logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Userhandles timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp userhandles --output=greptext --output-file %destinationDirectory%\volatility_userhandles.txt
        ExportFormat: greptext
