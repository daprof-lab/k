# ⚙️ **Volatility Connections**
### `File Name: Volatility_connections.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Connections to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Connections logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Connections timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp connections --output=greptext --output-file %destinationDirectory%\volatility_connections.txt
        ExportFormat: greptext
