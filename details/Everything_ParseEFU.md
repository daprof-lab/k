# ⚙️ **Everything Parse EFU**
### `File Name: Everything_ParseEFU.mkape`
...
* **Bulk Automated Parsing**: Run Everything Parse EFU to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Everything Parse EFU logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Everything Parse EFU timeline outputs to isolate exactly when unauthorized scripts were loaded.
