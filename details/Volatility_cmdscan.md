# ⚙️ **Volatility Cmdscan**
### `File Name: Volatility_cmdscan.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Cmdscan to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Cmdscan logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Cmdscan timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp cmdscan --output=greptext --output-file %destinationDirectory%\volatility_cmdscan.txt
        ExportFormat: greptext
