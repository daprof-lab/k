# ⚙️ **Kape Research Registry ELAM JSON**
### `File Name: KapeResearch_Registry_ELAM_JSON.mkape`
...
* **Bulk Automated Parsing**: Run Kape Research Registry ELAM JSON to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Kape Research Registry ELAM JSON logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Kape Research Registry ELAM JSON timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
```yaml
Description: 'RECmd: Convert ELAM Registry hive to JSON for research'
...
Processors:
    -
        ...
        CommandLine: -f %sourceFile% --kn ROOT --nl false --json %destinationDirectory% --jsonf ELAM.json
        ExportFormat: json
