# ⚙️ **Volatility Dlllist**
### `File Name: Volatility_dlllist.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Dlllist to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Dlllist logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Dlllist timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp dlllist --output=greptext --output-file %destinationDirectory%\volatility_dlllist.txt
        ExportFormat: greptext
