# ⚙️ **Volatility Psxview**
### `File Name: Volatility_psxview.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Psxview to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Psxview logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Psxview timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp psxview --output=greptext --output-file %destinationDirectory%\volatility_psxview.txt
        ExportFormat: greptext
