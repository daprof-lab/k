# ⚙️ **Volatility Shimcache**
### `File Name: Volatility_shimcache.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Shimcache to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Shimcache logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Shimcache timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp shimcache --output=greptext --output-file %destinationDirectory%\volatility_shimcache.txt
        ExportFormat: greptext
