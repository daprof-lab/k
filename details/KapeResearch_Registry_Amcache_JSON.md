# ⚙️ **Kape Research Registry Amcache JSON**
### `File Name: KapeResearch_Registry_Amcache_JSON.mkape`
...
* **Bulk Automated Parsing**: Run Kape Research Registry Amcache JSON to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Kape Research Registry Amcache JSON logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Kape Research Registry Amcache JSON timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceFile% --kn ROOT --nl false --json %destinationDirectory% --jsonf Amcache.json
        ExportFormat: json
