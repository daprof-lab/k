# ⚙️ **Power Shell Execute Usn Jrnl Rewind**
### `File Name: PowerShell_Execute-UsnJrnlRewind.mkape`
...
Execute-UsnJrnlRewind.ps1 - Execute usnjrnl_rewind.exe on MFT and UsnJrnl CSV from MFTEcmd to "rewind" the UsnJrnl and add their full path to UsnJrnl entries. Works on the module destination directory.

---

## 🔍 **Investigative Use-Cases**
* **Bulk Automated Parsing**: Run Power Shell Execute Usn Jrnl Rewind to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Power Shell Execute Usn Jrnl Rewind logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Power Shell Execute Usn Jrnl Rewind timeline outputs to isolate exactly when unauthorized scripts were loaded.
