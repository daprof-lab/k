# ⚙️ **Volatility malfind**
### `File Name: Volatility_malfind.mkape`
...
Analyzes memory pages to locate hidden code injections and shellcode blocks.

---

## 🔍 **Investigative Use-Cases**
* **Bulk Automated Parsing**: Run Volatility malfind to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility malfind logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility malfind timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp malfind --output=greptext --output-file %destinationDirectory%\volatility_malfind.txt
        ExportFormat: greptext
