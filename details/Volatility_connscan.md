# ⚙️ **Volatility Connscan**
### `File Name: Volatility_connscan.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Connscan to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Connscan logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Connscan timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp connscan --output=greptext --output-file %destinationDirectory%\volatility_connscan.txt
        ExportFormat: greptext
