# ⚙️ **Volatility Pstree**
### `File Name: Volatility_pstree.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Pstree to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Pstree logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Pstree timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp pstree --output=greptext --output-file %destinationDirectory%\volatility_pstree.txt
        ExportFormat: greptext
