# ⚙️ **Kape Research Registry NTUSER JSON**
### `File Name: KapeResearch_Registry_NTUSER_JSON.mkape`
...
* **Bulk Automated Parsing**: Run Kape Research Registry NTUSER JSON to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Kape Research Registry NTUSER JSON logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Kape Research Registry NTUSER JSON timeline outputs to isolate exactly when unauthorized scripts were loaded.
