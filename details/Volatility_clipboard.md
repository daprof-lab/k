...
### `File Name: Volatility_clipboard.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Clipboard to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Clipboard logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Clipboard timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp clipboard --output=greptext --output-file %destinationDirectory%\volatility_clipboard.txt
        ExportFormat: greptext
