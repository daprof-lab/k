# ⚙️ **Volatility Shellbags**
### `File Name: Volatility_shellbags.mkape`
...
* **Bulk Automated Parsing**: Run Volatility Shellbags to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Volatility Shellbags logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Volatility Shellbags timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceDirectory%\memory.dmp shellbags --output=greptext --output-file %destinationDirectory%\volatility_shellbags.txt
        ExportFormat: greptext
