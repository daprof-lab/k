# ⚙️ **Kape Research Registry Userdiff JSON**
### `File Name: KapeResearch_Registry_userdiff_JSON.mkape`
...
* **Bulk Automated Parsing**: Run Kape Research Registry Userdiff JSON to parse multiple folders containing acquired target evidence in a single instruction.
* **Structured Report Output**: Generate sorted, structured CSV/JSON databases from raw Kape Research Registry Userdiff JSON logs to index file anomalies.
* **Incident Impact Assessment**: Leverage Kape Research Registry Userdiff JSON timeline outputs to isolate exactly when unauthorized scripts were loaded.
...
Processors:
    -
        ...
        CommandLine: -f %sourceFile% --kn ROOT --nl false --json %destinationDirectory% --jsonf userdiff.json
        ExportFormat: json
